...
void isUp(vector<vector<int>> vct, int currentRow, int currentCol, vector<int> & randomVec, bool check){
    ...
    if(currentRow != vct.size()-1){
        ...
        else if(check == true && !sss.isEmpty()){   // it also checks cells that are popped while backtracking
...
void findPath(vector<string>ioVec, int entryCol, int entryRow, int exitCol, int exitRow, int count){
    ...
    while(find == false){
        ...
        for (int i = 0; i < ioVec.size(); ++i){ //searching for paths without walls
            ...
            if(ioVec[i].substr(0, idx -1) == "x=" + itoa(currX) + " y=" + itoa(currY)){
                ...
                else{   //backtrack operation just like i did while generating mazes
                    xxx.push(x.top());
                    xxx2.push(x2.top());
                    x.pop();
                    x2.pop();
        ...
        if(x.top() == exitCol && x2.top() == exitRow){  // find is true if exit points are found
            find = true;
        }
    }
...
int main() {
    ...
    while(count != kMaze){  // it generates mazes up to kMaze
        
        while(!s.isEmpty()){    //emptying stacks when switching to new maze
            s.pop();
            s2.pop();
        }
        while(!ss.isEmpty()){
        ...
        while(move == true){    // when move is true it means there are empty cells to visit
            ...
            if(randomVec.size() != 0){  // if an empty cell to go has been found
                ...
                for (int i = 0; i < ioVec.size(); ++i){ //Updates the walls of previously written cells
                    int idx = ioVec[i].find("l");
                    if(ioVec[i].substr(0, idx -1) == ioLine2){
                        if(randomCell == 1){    //randomCell getting 1 updates the cell above
                            int u = ioVec[i].find("u");
                            ioVec[i].at(u+2) = '0';
                        }
                        else if(randomCell == 2){   //randomCell getting 2 updates the cell on the right
                            int r = ioVec[i].find("r");
                            ioVec[i].at(r+2) = '0';
                        }
                        else if(randomCell == 3){   //randomCell getting 3 updates the cell below
                            int d = ioVec[i].find("d");
                            ioVec[i].at(d+2) = '0';
                        }
                        else if(randomCell == 4){   //randomCell getting 4 updates the cell on the left
                ...
                if(randomCell == 1){    //increases current row
                    currentRow++;
                    strRow = itoa(currentRow);
                ...
                else if(randomCell == 2){   //increases current column
                    currentCol++;
                    strCol = itoa(currentCol);
                    string ioLine2 = "x=" + strCol + " y=" + strRow + " l=0 r=1 u=1 d=1";
                    ioVec.push_back(ioLine2);
                }
                else if(randomCell == 3){   //decreases current row
                    currentRow--;
                    strRow = itoa(currentRow);
                ...
                else if(randomCell == 4){   //decreases current column
                    currentCol--;
                    strCol = itoa(currentCol);
                ...
                if(!isOver(s, s2, ss, ss2, sss, sss2, mRow, nCol)){ //checks whether all cells are visited or not
        ...
        temp << mRow << " " << nCol << endl;
        for (int i = 0; i < ioVec.size(); ++i){ // writing to txt files
            temp << ioVec[i] << endl;
        }
        
        count++;
        
        if(count == pathMaze){  // finding a path for the chosen maze
