...
struct PhoneRecord
{
    ...
    PhoneRecord(const string fn, const string ln, const string num, const string ci) : fname(fn), lname(ln), number(num), city(ci), fullname("")
    {
...
template <typename Comparable>
void merge(vector<Comparable> &a, int startPos, int midPos, int endPos)
{
    int startPos2 = midPos + 1;

    if (a[midPos] < a[startPos2] || a[midPos] == a[startPos2])
        return;

    while ((startPos < midPos || startPos == midPos) && (startPos2 < endPos || startPos2 == endPos))
    {
        if (a[startPos] < a[startPos2] || a[startPos] == a[startPos2])
            startPos++;
        else
        {
            int val = a[startPos2];
...
bool takeInput(vector<PhoneRecord> &list, string file)
{
    ifstream input;
    string line;
...
int main()
{
    ...
    takeInput(quickVector, file);
    takeInput(insertionVector, file);
    takeInput(mergeVector, file);
    takeInput(heapVector, file);
    ...
    {
        cout << insertionVector[firstIndex] << endl
             << endl;
    }
    else
    {
    ...
    cout << "Binary Search Time: " << binaryTime / N << " Nanoseconds" << endl
         << endl;
    ...
    cout << "Sequential Search Time: " << sequentialTime / N << " Nanoseconds" << endl
         << endl;
